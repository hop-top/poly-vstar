...
**`hop.top/vstar`** — canonical calendar and contact interchange for
agentic systems: RFC 5545 and RFC 6350, with byte-stable output and a
content hash. This module is the reference implementation: the bytes
every other implementation is measured against.
...
module is the Go reference implementation: the bytes every other
implementation — TypeScript, Python, Rust, PHP — is measured against.
...
calling a reordering a change. V\* fixes the equivalence class to one
representative: a **canonical form** — one byte sequence per logical
content — and an `X-VSTAR-HASH` computed over it turn "did this
change?" into `a == b` on two strings. On top it adds what
machine-generated state needs and a calendar library does not carry:
`validate` with diagnostic codes stable across minor versions,
`rrule` evaluation under a published iteration bound, structural
`diff` through canonical form, and append-only `supersession` that
records a status change as a new `VJOURNAL` pointing at the old
component rather than mutating it.

## Use this when

- You emit agent state — todos, journals, events, contacts — and want
  it to ride existing calendar, scheduler and contact tooling with no
  bespoke wire format.
- Consumers in other languages must agree with you byte-for-byte, and
  you want a hash that proves they did. The four ports reproduce this
  module's output over the whole conformance corpus, so a hash
  computed here verifies there; the quick start below prints the same
  `sha256:e551d177…` every implementation prints for that input.
...
In Go specifically: the module depends on the standard library and
`golang.org/x/text` only; "is this still the record I hashed?" is one
call, `hashing.VerifyXVSTAR`; and datetimes resolve only against the
document's own `VTIMEZONE`, never the host's tz database, so the same
document hashes identically on every machine in every year.
...
for RFC 5545's long tail. Use a general iCalendar library. V\*
...
Versions are prereleases (`1.0.0-alpha.*`); pin an exact version until
1.0.0.
