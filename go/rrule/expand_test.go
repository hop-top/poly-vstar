...
func TestOccurrences(t *testing.T) {
	cases := []occCase{
		...
		{
			// The limit-vs-expand pin for MINUTELY. BYHOUR and
			// BYMINUTE LIMIT under MINUTELY (RFC 5545 §3.3.10): from
			// 08:00 the minute bases 08:00..08:59 fail BYHOUR, 09:00
			// passes, 09:01..09:29 fail BYMINUTE, 09:30 passes, then
			// nothing until 2026-04-02 09:00. COUNT counts every
			// emission, so an evaluator that EXPANDS BYHOUR×BYMINUTE
			// inside each minute period re-emits 09:00/09:30 from
			// every base and exhausts COUNT on duplicates
			// ([09:00, 09:30, 09:00]) — a mistake NextOccurrence
			// cannot see because it filters by `after`.
			name:    "minutely_limits_count_complete",
			rrule:   "FREQ=MINUTELY;BYHOUR=9;BYMINUTE=0,30;COUNT=3",
