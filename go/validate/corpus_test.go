...
// valueDomainCodes are the spec/05 §8 diagnostics: every enumerated
// and integer value domain the validator bounds.
var valueDomainCodes = []string{
	validate.CodeStatusNotInVocabulary,
	validate.CodeClassNotInVocabulary,
	validate.CodeTranspNotInVocabulary,
	validate.CodeIntegerOutOfDomain,
}

// TestValueDomains_corpusIsClean walks every VCALENDAR fixture under
// testdata/rfc5545 and testdata/supersession and asserts none yields
// a §8 code: the conformance corpus carries only STATUS, CLASS and
// TRANSP values inside their vocabularies and only in-domain
// PRIORITY, PERCENT-COMPLETE and SEQUENCE values (spec/05 §8).
func TestValueDomains_corpusIsClean(t *testing.T) {
	flagged := map[string]bool{}
	for _, code := range valueDomainCodes {
		flagged[code] = true
	}
