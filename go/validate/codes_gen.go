...
const (
	...
	// integer (RFC 5545 §3.3.6 / §3.8.6.2) written as a canonical
	// decimal — no sign, no leading zeros, no whitespace (see spec/05
	// §8); or a TRIGGER whose VALUE parameter contradicts its value,
	// or that carries RELATED on an absolute trigger — see spec/03
	// §TRIGGER conventions. (spec/05 criterion 7)
	CodeMalformedDuration = "VS052"

	// CodeClassNotInVocabulary — CLASS value is outside the RFC 5545
	// §3.8.1.3 vocabulary PUBLIC, PRIVATE, CONFIDENTIAL (compared
	// case-insensitively — see spec/05 §8). (spec/05 criterion 8)
	CodeClassNotInVocabulary = "VS053"

	// CodeTranspNotInVocabulary — TRANSP value is outside the RFC 5545
	// §3.8.2.7 vocabulary OPAQUE, TRANSPARENT (compared
	// case-insensitively — see spec/05 §8). (spec/05 criterion 8)
	CodeTranspNotInVocabulary = "VS054"

	// CodeIntegerOutOfDomain — An integer-valued property is not a
	// canonical decimal (no sign, no leading zeros, no whitespace)
	// inside its RFC 5545 domain: PRIORITY 0–9 (§3.8.1.9),
	// PERCENT-COMPLETE 0–100 (§3.8.1.8), SEQUENCE non-negative
	// (§3.8.7.4) — see spec/05 §8. (spec/05 criterion 8)
	CodeIntegerOutOfDomain = "VS055"
)
...
var codeSeverities = map[string]Severity{
	...
	CodeClassNotInVocabulary:     SeverityError,
	CodeTranspNotInVocabulary:    SeverityError,
	CodeIntegerOutOfDomain:       SeverityError,
}
...
func Codes() []string {
	return []string{
		...
		CodeClassNotInVocabulary,
		CodeTranspNotInVocabulary,
		CodeIntegerOutOfDomain,
	}
}
