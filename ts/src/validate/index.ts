 ...
 *   canonical-decimal check is textual, so a SEQUENCE past every machine
 *   integer is well-formed. REPEAT's form is checked by the same rule
 *   but reported under the §7 code. Values are checked wherever the
 *   property appears; component scope is not diagnosed, and neither
 *   entry point descends into sub-components.
 *
...
import { checkIntegerDomains } from "./integer.js";
...
function runChecks(c: Component, path: string, ledger: readonly Component[] | undefined): Diagnostic[] {
  return [
    ...
    ...checkClassification(c, path),
    ...checkIntegerDomains(c, path),
