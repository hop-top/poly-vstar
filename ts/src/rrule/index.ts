 ...
 *
 * The accepted scope is fixed by spec/v0.1/03 §RRULE parsing scope:
 * `FREQ` of `HOURLY`, `DAILY`, `WEEKLY`, `MONTHLY` or `YEARLY`, with
 * `INTERVAL`, `UNTIL` (UTC form #2 only), `COUNT`, every `BY-*` clause,
 * and `WKST`. `FREQ=SECONDLY`, `FREQ=MINUTELY` and `RSCALE` parse as
