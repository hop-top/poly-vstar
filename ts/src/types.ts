...
/**
 * The wire-string component type of a {@link Component}.
 *
 * The type is deliberately **open**, like {@link RelType}: the
 * constants below name the registered vocabulary, they do not bound
 * it. RFC 5545 §3.6.5 nests `STANDARD` and `DAYLIGHT` inside a
 * `VTIMEZONE`, and `X-`-prefixed components are permitted, so any
 * wire token is a valid `CompType`. The `string & {}` tail keeps the
 * literals distinct for autocomplete and `Extract` while admitting
 * every other string; a caller narrowing on one of the constants gets
 * `<literal> | (string & {})`, not the bare literal.
 *
 * No `VCARD` constant exists — vCards are {@link Card}s, not
 * Components.
 */
 ...
 *
 * Open like {@link CompType}: the registry accepts further values and
 * the RFC permits `x-name` tokens, so the constants name the
 * registered vocabulary without bounding it.
 */
export type Kind = "individual" | "org" | "group" | "" | (string & {});
