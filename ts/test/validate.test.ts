...
import { COMP_ALARM, COMP_JOURNAL, COMP_TODO, TRANSP_OPAQUE } from "../src/types.js";
...
describe("validate — corpus is clean of every §8 value-domain diagnostic", () => {
  // Mirrors the Go corpus walk: every VCALENDAR fixture in the
  // conformance corpus carries only STATUS, CLASS and TRANSP values
  // inside their vocabularies and only canonical, in-range PRIORITY,
  // PERCENT-COMPLETE and SEQUENCE values. A hit here means the port's
  // rule is wrong, not the fixture.
  const SECTION_8_CODES: readonly Code[] = [
    CODES.CodeStatusNotInVocabulary,
    CODES.CodeClassNotInVocabulary,
  ...
  for (const family of ["rfc5545", "supersession"]) {
    ...
    for (const name of readdirSync(dir).filter((n) => n.endsWith(".ics")).sort()) {
      it(`${family}/${name}`, () => {
        ...
        const hits = validate(cal).filter((d) => SECTION_8_CODES.includes(d.code));
        expect(hits.map((d) => `${d.code} ${d.path}`)).toEqual([]);
      });
    }
  }
...
// spec/behavior/validate/ spells its property names in uppercase, so a
// port could fold names wrongly and still pass the whole behavior gate;
// the §8 value rules are pinned there on one component type each, so a
// port that gated them by type would pass too.
//
...
describe("validate — rules the behavior fixtures reach only weakly", () => {
  ...
  /**
   * A minimally-valid component of type `type` carrying one extra
   * property, so the only interesting diagnostic is the one the
   * property provokes. The UID is fixed so paths are predictable.
   */
  function withValue(type: string, name: string, value: string): Component {
    const props = [
      prop("UID", "value-1"),
      prop("DTSTAMP", "20260504T120000Z"),
      prop("X-VSTAR-HASH", "sha256:abc"),
    ...
    if (type === COMP_TODO) props.push(prop("DUE", "20260601T000000Z"));
    return { type, props, sub: [] };
  }

  /** The paths of every diagnostic carrying `code`. */
  function pathsOf(c: Component, code: Code): string[] {
    return validateComponent(c)
      .filter((d) => d.code === code)
      .map((d) => d.path);
  }

  const INTEGER_PROPS = ["PRIORITY", "PERCENT-COMPLETE", "SEQUENCE"];

  it("flags CLASS outside the vocabulary on a non-VEVENT — scope is not gated", () => {
    // The shared fixture carries CLASS on a VEVENT only. The value is
    // checked wherever the property appears; V* diagnoses no scope rule.
    for (const type of [COMP_TODO, COMP_JOURNAL]) {
      expect(pathsOf(withValue(type, "CLASS", "X-SECRET"), CODES.CodeClassNotInVocabulary)).toEqual([
        `${type}[uid=value-1].CLASS`,
      ]);
    }
  });

  it("flags TRANSP outside the vocabulary on a VTODO, and accepts OPAQUE there", () => {
    // RFC 5545 admits TRANSP on VEVENT only, but scope is not diagnosed:
    // a VTODO gets the same value check and nothing else.
    expect(pathsOf(withValue(COMP_TODO, "TRANSP", "BUSY"), CODES.CodeTranspNotInVocabulary)).toEqual([
      "VTODO[uid=value-1].TRANSP",
    ]);
    expect(pathsOf(withValue(COMP_TODO, "TRANSP", TRANSP_OPAQUE), CODES.CodeTranspNotInVocabulary)).toEqual([]);
  });

  it("flags whitespace-padded integers — a trim would silently accept them", () => {
    for (const name of INTEGER_PROPS) {
      for (const value of [" 3", "3 "]) {
        expect(pathsOf(withValue(COMP_TODO, name, value), CODES.CodeIntegerOutOfDomain)).toEqual([
          `VTODO[uid=value-1].${name}`,
        ]);
      }
    }
  });

  it("decides well-formedness textually, never by an integer parse", () => {
    // `Number("+3")` and `Number("07")` both read as in-range integers,
    // so a parse-based check accepts them; the canonical-decimal rule
    // rejects both. And 2^64 is past every machine integer, yet is a
    // well-formed SEQUENCE — the spec bounds SEQUENCE below, never above.
    for (const name of INTEGER_PROPS) {
      for (const value of ["+3", "07"]) {
        expect(pathsOf(withValue(COMP_TODO, name, value), CODES.CodeIntegerOutOfDomain)).toEqual([
          `VTODO[uid=value-1].${name}`,
        ]);
      }
    }
    expect(pathsOf(withValue(COMP_TODO, "SEQUENCE", "18446744073709551616"), CODES.CodeIntegerOutOfDomain)).toEqual(
      [],
    );
  });
