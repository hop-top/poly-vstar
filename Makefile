...
.PHONY: \
	lint-ts typecheck-ts test-ts build-ts package-ts ci-ts \
...
typecheck-ts: ## Type-check the TypeScript port, tests included (tsc --noEmit)
	cd ts && $(PNPM) install --ignore-scripts && $(PNPM) typecheck
...
ci-ts: lint-ts typecheck-ts test-ts build-ts ## TypeScript port gate (lint + typecheck + test + build)
