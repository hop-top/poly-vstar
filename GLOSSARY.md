...
| **Canonical form** | The deterministic byte sequence for a component, used for equality + hashing. See [`03-canonicalization.md`](spec/v0.1/03-canonicalization.md). |
| **Supersession** | Append-only state change pattern using `RELATED-TO` + `X-VSTAR-EFFECTIVE-STATUS`. See [`02-component-mapping.md`](spec/v0.1/02-component-mapping.md). |
...
| **Conformance corpus** | The canonical fixture set under [`spec/v0.1/conformance/`](spec/v0.1/conformance/) that every implementation is checked against. [`go/testdata/`](go/testdata/) is its generated mirror. |
