...
class Freq(StrEnum):
    ...
    syntactically valid RFC values this scope defers, so they are
    rejected at parse with :class:`~vstar.UnsupportedRrule` rather than
    modelled — a member for each would invite an evaluator branch the
    spec says must not exist.
    """
