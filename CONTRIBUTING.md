...
- Spec **prose** — `spec/v*/**/*.md` — ships under **CC-BY-4.0**;
  ...
  so it never renders. The conformance corpus
  (`spec/v*/conformance/**`) and the spec's release tooling
  (`spec/.github/scripts/**`) ship under **MIT**, so an
  implementation can vendor the fixtures on the same terms as the
  code that consumes them. Fixtures carry **no** SPDX header: they
  are byte-exact test data, and a header line would be part of the
  input.
