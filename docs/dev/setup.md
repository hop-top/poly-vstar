...
A green run ends on the parity line:

```text
parity: ok go php py rs ts (184 cases)
```
...
- **Every seed in `.release-please-manifest.json` is
  prerelease-shaped** (`0.1.0-alpha.0`, never `0.0.0`). Each package
  declares `prerelease`, and the org preflight rejects a
  stable-shaped seed for a prerelease component. The first cut from
  an `alpha.0` seed lands at `alpha.1`.
