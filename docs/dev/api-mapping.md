...
`Freq`, `Weekday` and `Severity`. A backed enum's wire string also remains
reachable as `->value`. Value classes that are NOT enums — `VDate`,
`VDuration`, `RelType`, `ComponentDiff`, `Rule`, `RecurrenceRange` — keep
...
| `(*VCalendarParser) Header() vstar.Calendar` | `header(): Calendar` | `header() -> Calendar` | `header(&self) -> &Calendar` | `header(): Calendar` |
...
| `(Range) String() string` | `toString()` | `__str__()` | `Display for Range` | `__toString()` |
...
`Freq` values: `FreqInvalid` (zero value), `FreqHourly`, `FreqDaily`,
`FreqWeekly`, `FreqMonthly`, `FreqYearly`. `FreqInvalid` MUST be the
...
| `ErrUnsupportedRRule` | `rrule` | Syntactically valid but outside the RRULE parsing scope: `FREQ=SECONDLY`, `FREQ=MINUTELY`, `RSCALE`, or a non-UTC `EXDATE`/`RDATE`/`RECURRENCE-ID` value. |
