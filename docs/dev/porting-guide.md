...
2. **A green corpus gate is not coverage.** `behavior/validate` cannot
   reach case-insensitive property matching, the `TRIGGER` / `REPEAT`
   arms of the duration rule, or a present-but-blank supersession
   property — the fixtures are all-uppercase, reach that code only
   through `DURATION`, and never spell an empty value. Three lanes had
   to add unit tests for these *after* the gate was green, and a fourth
   gap — the positional `[#n]` counter, which every fixture left frozen
   at `[#0]` — was found by mutation testing in three lanes before a
   shared fixture closed it. Write the tests while you are in the
   layer, not after. See [layer (d)](#layer-d--validation).
...
| **d** | `validate` with generated registry constants | `behavior/validate` passes; the conformance corpus is clean of the status-vocabulary diagnostic |
...
names in uppercase, the malformed-duration code is reached only through
`DURATION`, and no fixture carries a present-but-blank `RELATED-TO` or
`X-VSTAR-EFFECTIVE-STATUS` (the reference treats a whitespace-only
value as absent). So case-insensitive name matching, the `TRIGGER` /
`REPEAT` arms of the duration rule and blank-value handling all pass
the gate whether or not a port gets them right. Each port carries its
own unit tests for these; yours should too. The positional `[#n]`
counter used to be on this list — every fixture had one UID-less
component per type, so a counter frozen at `[#0]` passed — until
`missing_uid_positional` added two; the shared fixture now pins it for
every port.
...
Plus: the whole conformance corpus MUST be clean of the
status-vocabulary diagnostic (the code covering a `STATUS` value
outside the vocabulary RFC 5545 §3.8.1.11 scopes to the component's own
type). If your port reports it on a corpus fixture, your status-scoping
is wrong, not the fixture's.
...
in it, which is why the layer-(d) gate above describes the
status-vocabulary diagnostic by its meaning instead of naming it. Do
