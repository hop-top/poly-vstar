...
unpublish or delete. If `0.5.0-alpha.0` goes out with a broken build,
that number is burned; the fix ships as `0.5.0-alpha.1`.
...
`vstar-php`, and `vstar-v0.1` (the spec tree). Read the actual titles
...
The Go (`vstar`) and spec (`vstar-v0.1`) components need no registry
