...
|---|---|---|
| `ParseRRule` returns `ErrUnsupportedRRule` for a feature you need | Feature is deferred (`FREQ=SECONDLY`, `FREQ=MINUTELY`, `RSCALE`). | These are intentionally out of v0.2 scope per [spec/03 §RRULE parsing scope](../../spec/v0.1/03-canonicalization.md#rrule-parsing-scope). Open an issue with your use case to drive a v0.3 scope review. |
