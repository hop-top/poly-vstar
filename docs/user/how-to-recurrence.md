...
- You want to drop a non-vstar iCal library kept solely for
  `ParseRecurrenceRule` (the v0.2 motivating use case).
...
| `_, rrule.ErrUnsupportedRRule` | Syntactically valid but uses a feature outside v0.2 scope (`FREQ=SECONDLY`, `RSCALE`). | Fall back: store the raw string and skip evaluation. The validator emits `VS050` (Warning) for this case. |
...
|---|---|---|
| `ParseRRule` returns `ErrUnsupportedRRule` for a feature you need | Feature is deferred (`FREQ=SECONDLY`, `RSCALE`). | These are intentionally out of v0.2 scope per [spec/03 §RRULE parsing scope](../../spec/v0.1/03-canonicalization.md#rrule-parsing-scope). Open an issue with your use case to drive a v0.3 scope review. |
...
| `UNTIL=20261231` returns `ErrMalformed` | V* requires UNTIL in RFC 5545 form #2 (UTC, `Z`-suffixed) — see [spec/03 §RRULE parsing scope](../../spec/v0.1/03-canonicalization.md#rrule-parsing-scope). | Use `UNTIL=20261231T235959Z`. |
...
- `FREQ=SECONDLY` — extreme expansion, no agentic use case in v0.2.
- `RSCALE` (RFC 7529, non-Gregorian calendars) — out of V* roadmap
  indefinitely.

The full v0.2 scope lives in
...
- [Specification §03 — RRULE parsing scope](../../spec/v0.1/03-canonicalization.md#rrule-parsing-scope) —
