...
You are importing the Go reference (`hop.top/vstar`) or one of its
four ports into your project, OR building a sister implementation in
another language that must agree byte-for-byte with the Go reference.

- [Quickstart](user/quickstart.md) — parse your first VCALENDAR in
  five minutes.
- [Go module](../go/README.md) — `hop.top/vstar`, the reference
  implementation every port is measured against: install, package
  layout, and the API surface for Go 1.25+.
