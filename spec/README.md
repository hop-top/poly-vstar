...
> `vstar-v0.1/v*` tag. The mirror comes into existence with the first
...
| What | Paths | Licence |
|------|-------|---------|
| Spec text | `v*/**/*.md` | CC-BY-4.0 |
| Conformance corpus and release tooling | `v*/conformance/**`, `.github/scripts/**` | MIT |

Every CC-BY-4.0 file carries an `SPDX-License-Identifier` header, so
the scoping survives a file being copied out of the tree. Everything
outside this tree — the implementations and the contributor
documentation — is MIT under the source repository's own licence.
