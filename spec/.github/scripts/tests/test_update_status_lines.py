...
class UpdateStatusLinesTests(unittest.TestCase):
    ...
    def test_alpha_maps_to_draft(self) -> None:
        r = _run_script(
            self.work, "chore(release): vstar-v0.1 0.1.0-alpha.0", "release-pr",
        )
    ...
    def test_beta_maps_to_pre_release(self) -> None:
        r = _run_script(
            self.work, "chore(release): vstar-v0.1 0.1.0-beta.2", "release-pr",
        )
        self.assertEqual(r.returncode, 0, r.stderr)
    ...
    def test_rc_maps_to_release_candidate(self) -> None:
        r = _run_script(
            self.work, "chore(release): vstar-v0.1 0.1.0-rc.0", "release-pr",
        )
        self.assertEqual(r.returncode, 0, r.stderr)
        lines = _status_lines(self.work)
        self.assertEqual(
    ...
    def test_stable_maps_to_general_availability(self) -> None:
        r = _run_script(
            self.work, "chore(release): vstar-v0.1 0.1.0", "release-pr",
        )
        self.assertEqual(r.returncode, 0, r.stderr)
        lines = _status_lines(self.work)
        self.assertEqual(
    ...
    def test_branch_with_double_dot_exits_2(self) -> None:
        r = _run_script(
            self.work, "chore(release): vstar-v0.1 0.1.0-alpha.0", "foo/../bar",
        )
        self.assertEqual(r.returncode, 2)
    ...
    def test_branch_with_semicolon_exits_2(self) -> None:
        r = _run_script(
            self.work, "chore(release): vstar-v0.1 0.1.0-alpha.0", "main; rm",
        )
        self.assertEqual(r.returncode, 2)

    def test_branch_with_newline_exits_2(self) -> None:
        r = _run_script(
            self.work, "chore(release): vstar-v0.1 0.1.0-alpha.0", "foo\nbar",
        )
        self.assertEqual(r.returncode, 2)
