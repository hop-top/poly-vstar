...
Inputs (env, strictly validated):
  PR_TITLE  — e.g. `chore(release): vstar-v0.1 0.1.0-alpha.0`
...
# Component shape: vstar-v0.1, vstar-v1.0, etc.
# Version shape: 0.1.0, 0.1.0-alpha.0, 0.2.3-rc.4, etc.
TITLE_RE = re.compile(
    r"^chore\(release\):\s*"
    r"(?P<component>vstar-v\d+\.\d+)\s+"
...
def spec_dir_for(component: str) -> Path:
    # component is `vstar-vX.Y` → directory is `spec/vX.Y`
    suffix = component[len("vstar-"):]  # "vX.Y"
    p = SPEC_ROOT / suffix
    if not p.is_dir():
