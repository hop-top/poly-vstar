...
uppercase, so case-insensitive name matching is untested (values are
covered: `clean_vocabulary_lowercase` carries a lowercase `CLASS` and
`TRANSP`); no fixture carries a present-but-blank `RELATED-TO` or
`X-VSTAR-EFFECTIVE-STATUS`, which the reference treats as absent;
whitespace-padded integer values (`PRIORITY: 3`, or a `SEQUENCE` whose
value carries a trailing space) are not fixtured, because whether a
port's content-line parser preserves the padding is a codec question
this family must not pin; component
scope of `CLASS`, `TRANSP`, `PRIORITY`, `PERCENT-COMPLETE`,
`SEQUENCE` and `REPEAT` is not diagnosed by any implementation, so
nothing here exercises it; and sub-components are not validated —
`Validate` does not descend into a component's children, so nothing
inside a nested `VALARM` is reachable. The `repeat_*` and
`malformed_trigger` fixtures carry a **top-level** `VALARM` for that
reason, as `vcard_missing_required` carries a top-level `VCARD`; it
is structurally odd iCalendar and deliberate. A port pins the rest
