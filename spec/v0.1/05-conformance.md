...
- **Emitter**: produces V\* documents from a system's own data model.
...
| Unsupported RRULE | `ErrUnsupportedRRule` | Syntactically valid input outside the RRULE parsing scope: `FREQ=SECONDLY`, `RSCALE`, or a non-UTC `EXDATE`/`RDATE`/`RECURRENCE-ID` value. |
...
Those deviations are confined to API surface — error shape, instant
