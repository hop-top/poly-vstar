...
specification. Every implementation — the Go reference and the
TypeScript, Python, Rust and PHP ports, plus future AGR-Racket
cross-validation — consumes these fixtures identically: same input,
same canonical bytes, same hash. CI asserts the property; drift fails
the PR.
...
Each port and any future AGR-Racket V\* implementation MUST produce
byte-identical `.canonical` and `.hash` content for these fixtures.
