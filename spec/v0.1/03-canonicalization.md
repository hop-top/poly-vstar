...
- `FREQ` — required; `HOURLY`, `DAILY`, `WEEKLY`, `MONTHLY`, `YEARLY`.
...
Evaluator behaviour: `FREQ=HOURLY|DAILY|WEEKLY|MONTHLY|YEARLY` with
any combination of the accepted rule-parts; WKST-aware week
...
value is implementation-defined. Reaching the bound MUST be
...
against it, emitter for emitter, in parity CI. AGR's Racket emitter
cross-validates against the Go output (non-blocking for v0.1.0).
