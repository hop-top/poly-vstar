...
|-----------|------|-----|--------|
| Spec | `spec/` | `vstar-spec/v*` | [`hop-top/spec-vstar`](https://github.com/hop-top/spec-vstar) |
...
files under `spec/` carry none — fixtures are byte-exact test data and
JSON has no comment syntax — so a spec-tree file with no header is
MIT, not unscoped.
