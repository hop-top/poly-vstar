 ...
 *   scope: FREQ=SECONDLY, FREQ=MINUTELY, RSCALE.
 *
...
final class Parser
{
    ...
    /**
     * The `FREQ` token, rejecting the two deferred frequencies by name.
     *
     * The distinction matters: an unrecognized token is malformed, while
     * `SECONDLY` and `MINUTELY` are valid RFC values this scope defers,
     * and the corpus asserts which answer each gets.
     */
    private static function parseFreq(string $v): Freq
    {
        switch ($v) {
            ...
            case 'YEARLY':
                ...
                // of the five, so the total `from` is honest here where a
