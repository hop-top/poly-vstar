...
final class Evaluator
{
     ...
     * evaluated from February) reports `ErrIterationCap` rather than
     * the eventual occurrence. Skipping the base to the next window a
    ...
    public static function advance(Rule $rule, \DateTimeImmutable $current): ?\DateTimeImmutable
    {
        ...
        switch ($rule->freq) {
            case Freq::Minutely:
                return $current->modify('+' . $rule->interval . ' minutes');
    ...
    private static function expand(Rule $rule, \DateTimeImmutable $base, \DateTimeImmutable $dtstart): array
    {
        return match ($rule->freq) {
            Freq::Minutely => self::expandMinutely($rule, $base, $dtstart),
    ...
    /**
     * One base minute, at BYSECOND of that minute.
     *
     ...
     * BYSECOND entry, the base's second when unset, on the base's date,
     * hour and minute -- the {@see self::expandHourly()} shape one
     * level down.
     *
     * @return list<\DateTimeImmutable>
     */
    private static function expandMinutely(Rule $rule, \DateTimeImmutable $base, \DateTimeImmutable $dtstart): array
    {
        if (!self::matchesByMonth($rule, $base)
            || !self::matchesByMonthDay($rule, $base)
            || !self::matchesByDay($rule, $base)
            || !self::matchesByHour($rule, $base)
            || !self::matchesByMinute($rule, $base)) {
            return [];
        }
        ...
        foreach (self::byOrDefault($rule->bySecond, $b['second']) as $second) {
            $out[] = self::fromWall([
                'year' => $b['year'],
                'month' => $b['month'],
                'day' => $b['day'],
    ...
    /**
     * One base hour, at BYMINUTE x BYSECOND of that hour.
     *
     ...
     * in BYHOUR yields nothing; one that passes emits one occurrence per
     * BYMINUTE x BYSECOND pair, the base's minute and dtstart's second
     * when unset.
     *
     * @return list<\DateTimeImmutable>
     */
    private static function expandHourly(Rule $rule, \DateTimeImmutable $base, \DateTimeImmutable $dtstart): array
    {
        ...
        // BYMONTH/BYMONTHDAY/BYDAY/BYHOUR limit; BYMINUTE×BYSECOND expand
     ...
     *
     * The {@see self::matchesByMonth()} shape, used where the RFC 5545
     * §3.3.10 table says BYHOUR limits: HOURLY and finer.
     */
    private static function matchesByHour(Rule $rule, \DateTimeImmutable $t): bool
    {
        if ($rule->byHour === []) {
            return true;
        }

        return in_array(self::toWall($t)['hour'], $rule->byHour, true);
    }

    /**
     * Whether `$t`'s minute satisfies BYMINUTE (vacuously true when
     * unset).
     *
     * The BYMINUTE twin of {@see self::matchesByHour()}, used where the
     * table says BYMINUTE limits: MINUTELY.
     */
    private static function matchesByMinute(Rule $rule, \DateTimeImmutable $t): bool
    {
        if ($rule->byMinute === []) {
            return true;
        }

        return in_array(self::toWall($t)['minute'], $rule->byMinute, true);
    }
