 ...
 *   canonical-decimal check is textual, so a SEQUENCE past `PHP_INT_MAX`
 *   is well-formed. REPEAT's form is checked by the same rule but
 *   reported under the malformed-duration code. Values are checked
 *   wherever the property appears; component scope is not diagnosed, and
 *   sub-components are not descended into.
 *
...
final class Validate
{
    ...
    private static function run(Component $c, string $path, ?array $ledger): array
    {
        return array_merge(
            ...
            Types::check($c, $path),
            Status::check($c, $path),
            Classification::check($c, $path),
            Integer::check($c, $path),
