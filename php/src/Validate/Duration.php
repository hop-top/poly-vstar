...
final class Duration
{
    ...
    private static function checkProperty(Property $p, string $path): ?Diagnostic
    {
        ...
        // The same textual canonical-decimal rule the bounded integers
        // follow (spec/05 §8): a sign or a leading zero is malformed even
        // though the number is in range. REPEAT stays under the
        // malformed-duration code because a published code never changes
        // meaning.
        if (Internal::equalFold($p->name, self::REPEAT) && !Integer::isCanonicalDecimal($p->value)) {
