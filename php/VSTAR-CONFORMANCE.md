...
1. **Enums spell the display method `toString()`, not `__toString()`.**
   ...
   `Severity`, `Scope`, `DiffOp`, `CompType`, `Kind` and the status
   enums therefore expose the reference's `String()` as a plain
   `toString()` method. Non-enum value classes — `VDuration`,
   `Rrule\Rule`, `Diff\ComponentDiff` — keep `__toString()`. The
   contract in every case is the backing `->value`, which is the wire
   token the corpus compares; `toString()` is display text.
