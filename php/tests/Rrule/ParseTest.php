 ...
 *   exactly one thing: the value parses. Eighteen of them.
...
final class ParseTest extends TestCase
{
    ...
    /**
     * `FREQ=MINUTELY` is in scope: it parses to its own enum case, and
     * that case renders its RFC wire spelling. The corpus pins the
     * parse (`happy/freq_minutely`) and the emitted form
     * (`format/minutely_interval`) but never names the case, so this
     * is the assertion that ties the token to `Freq::Minutely`.
     */
    public function testMinutelyParsesToItsEnumCase(): void
    {
        $rule = Rrule::parse('FREQ=MINUTELY');

        self::assertSame(Freq::Minutely, $rule->freq);
        self::assertSame('MINUTELY', Freq::Minutely->toString());
        self::assertSame('FREQ=MINUTELY', (string) $rule);
    }
