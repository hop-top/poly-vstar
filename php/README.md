...
**`hop-top/vstar`** on Packagist — canonical calendar and contact
...
package is the PHP implementation; the Go reference and the
TypeScript, Python and Rust ports produce the same bytes for the same
input.
...
this change?" is `===` on two strings rather than a tree walk. On top
it adds what agent state needs and a calendar library does not carry: a
...
In a PHP project specifically: PSR-4 under `HopTop\Vstar\`,
`declare(strict_types=1)` throughout, and static analysis at PHPStan
level 9, so the types the package declares are the types you get.
...
`sabre/vobject`. V\* deliberately implements a bounded subset chosen
for machine-generated state: its `RRULE` scope excludes
`FREQ=SECONDLY` and `RSCALE`, and its vCard codec accepts
version 4.0 only. If you simply want to read someone else's calendar
file, this is more machinery than you need.

## Install

The package ships alpha versions (`1.0.0-alpha.*`) until 1.0.0, and
...
version: `"hop-top/vstar": "1.0.0-alpha.1"` <!-- x-release-please-version -->
...
the contract — it is the token the corpus compares. Every enum in the
package also declares `toString(): string`. Where the reference gives a
type a `String()` — `Severity`, `Scope`, `DiffOp`, `Related`, `Freq`,
`Weekday` and `RecurrenceRange` — `toString()` renders that display
spelling; the enums the reference leaves without one — `CompType`,
`Kind`, `VClass`, `Transp`, `EventStatus`, `TodoStatus` and
`JournalStatus` — return `->value`, so no caller has to remember which
enums are stringable. The method is spelled `toString()`, not
`__toString()`, because PHP rejects the magic method on an enum at
declaration time. Non-enum value classes — `VDuration`, `Rrule\Rule`,
`Rrule\ByDay`, `Diff\ComponentDiff`, `VDate`, `RelType` — keep
`__toString()`, so `(string) $rule` works as usual.
