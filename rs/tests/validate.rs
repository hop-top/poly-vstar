...
#[test]
fn the_conformance_corpus_is_clean_of_the_status_vocabulary_diagnostic() {
    ...
    for family in ["rfc5545", "supersession"] {
        ...
        for case in support::cases_in(&dir, "ics") {
            ...
            let hits: Vec<String> = validate(&cal)
                .iter()
                .filter(|d| d.code == status_code)
                .map(|d| d.path.clone())
                .collect();
            assert!(
                hits.is_empty(),
                "{family}/{}: unexpected {status_code} at {hits:?}",
                case.stem
            );
            checked += 1;
        }
