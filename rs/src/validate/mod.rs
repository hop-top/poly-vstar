...
//! - §8 enumerated and integer value domains — `STATUS` against the
//!   vocabulary RFC 5545 §3.8.1.11 scopes to the component's own type;
//!   `CLASS` and `TRANSP` against their closed vocabularies
//!   (§3.8.1.3, §3.8.2.7); `PRIORITY`, `PERCENT-COMPLETE` and
//!   `SEQUENCE` as canonical decimals inside their domains (0–9,
//!   0–100, non-negative). Vocabularies compare case-insensitively;
//!   the canonical-decimal check is textual, so a `SEQUENCE` past any
//!   machine-integer width is well-formed. `REPEAT`'s form is checked
//!   by the same rule but reported under §7's code. Values are checked
//!   wherever the property appears: component scope is not diagnosed,
//!   and validation does not descend into sub-components.
//!
//! # Path syntax
//!
...
fn run_checks(c: &Component, path: &str, ledger: Option<&[Component]>) -> Vec<Diagnostic> {
    ...
    out.extend(classification::check(c, path));
    out.extend(integer::check(c, path));
