...
pub(crate) fn advance(rule: &Rule, current: DateTime<Utc>) -> Option<DateTime<Utc>> {
    ...
    match rule.freq {
        Freq::Minutely => current.checked_add_signed(Duration::minutes(n)),
...
fn expand(rule: &Rule, base: DateTime<Utc>, dtstart: DateTime<Utc>) -> Vec<DateTime<Utc>> {
    match rule.freq {
        Freq::Minutely => expand_minutely(rule, base, dtstart),
...
/// `BYSECOND` entry, at base's second when unset, on base's date, hour
/// and minute — the `expand_hourly` shape one level down.
fn expand_minutely(
    rule: &Rule,
    base: DateTime<Utc>,
    ...
    if !matches_by_month(rule, base)
        ...
        || !matches_by_hour(rule, base)
        || !matches_by_minute(rule, base)
    {
        return Vec::new();
    }
...
/// `BYMINUTE×BYSECOND` pair, at base's minute and second when unset.
...
/// `BYHOUR` as a limit — the check for the periods where the RFC 5545
/// §3.3.10 table says it limits (`HOURLY` and finer). True when the
/// clause is absent. Same shape as [`matches_by_month`].
fn matches_by_hour(rule: &Rule, t: DateTime<Utc>) -> bool {
    rule.by_hour.is_empty()
        || rule
            .by_hour
            .iter()
            .any(|h| u32::try_from(*h) == Ok(t.hour()))
}

/// `BYMINUTE` as a limit — the `BYMINUTE` twin of [`matches_by_hour`],
/// for the periods where the table says it limits (`MINUTELY`).
fn matches_by_minute(rule: &Rule, t: DateTime<Utc>) -> bool {
    rule.by_minute.is_empty()
        || rule
            .by_minute
            .iter()
            .any(|m| u32::try_from(*m) == Ok(t.minute()))
}
