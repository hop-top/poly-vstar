...
//!
//! Accepted: `FREQ` (`HOURLY`, `DAILY`, `WEEKLY`, `MONTHLY`, `YEARLY`),
//! `INTERVAL`, `UNTIL` (form #2 / UTC only), `COUNT`, `BYDAY`,
//! `BYMONTH`, `BYMONTHDAY`, `BYHOUR`, `BYMINUTE`, `BYSECOND`,
//! `BYYEARDAY` and `BYWEEKNO` (`FREQ=YEARLY` only), `BYSETPOS`, `WKST`.
//!
//! Deferred, and reported as [`Error::UnsupportedRRule`]:
//! `FREQ=SECONDLY`, `FREQ=MINUTELY`, and `RSCALE` (RFC 7529).
