...
A generic iCalendar crate parses your `.ics` and hands back a tree. It
...
at any column. Serialize the same calendar twice and you can get
different bytes.
...
"did this change?" is a byte comparison rather than a tree walk. On top
it adds what agentic state needs and a calendar library does not carry:
a `validate` pass with stable diagnostic codes, a bounded `RRULE`
evaluator, structural `diff`, and append-only `supersession`.

Reach for this crate over a generic iCalendar crate when you need:

- **Byte-identical output across runtimes.** This port is checked
  byte-for-byte against the Go reference over the shared conformance
  corpus — agreement is measured, not assumed. See
  [Conformance](#conformance).
- **Content addressing.** A hash over canonical bytes, so documents can
  be deduplicated, cached, or compared across services. The quick start
  below prints the same `sha256:e551d177…` the Go, TypeScript, Python
  and PHP implementations print for the same input.
- **Append-only state.** Supersession chains rather than mutation, so
  history survives.

What a Rust reader gets specifically: `#![forbid(unsafe_code)]`; three
...
iCalendar crate. V\* deliberately implements a bounded subset chosen
